...
def plot_perceptron(neg_examples, pos_examples, mistakes0, mistakes1, num_err_history, w, w_dist_history):
    ...
    neg_correct_ind = list(set(range(neg_examples)) - set(mistakes0))  
    pos_correct_ind = list(set(range(pos_examples)) - set(mistakes1))  


    plt.subplot(221)

	plt.scatter(neg_examples[neg_correct_ind, 1], neg_examples[neg_correct_ind, 2], c='g', marker='o')

	plt.scatter(pos_examples[pos_correct_ind,1], pos_examples[pos_correct_ind, 2], c='g', marker='s')

    if len(mistakes0) > 0:
	    plt.scatter(neg_examples[mistakes0, 1], neg_examples[mistakes0, 2], c='r', marker='o')

    if len(mistakes1) > 0:
	    plt.scatter(pos_examples[mistakes1, 1], pos_examples[mistakes1, 2], c='r', marker='s')

    plt.title('Classifier');

    #In order to plot the decision line, we just need to get two points.
    plt.plot([-5, 5],[(-w[-1]+5*w[1])/w[2], (-w[-1]-5*w[1])/w[2]], 'k')
    xlim([-1,1]);
    ylim([-1,1]);


    plt.subplot(222);
    plt.plot(range(len(num_err_history)-1), num_err_history)
	...
	plt.subplot(223)
	plt.plot(range(len(w_dist_history)-1), w_dist_history)
	plt.xlim(-1, max(15, len(num_err_history)))
	plt.ylim(0, 15)
	plt.title('Distance')
	plt.xlabel('Iteration')
